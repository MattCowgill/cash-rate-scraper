...
page1 <- dict$PageTables$`1`
page1_tables <- page1[[1]]
cash_rate_table <- page1_tables$Data

# Convert row 1 to dates and row 2 to numbers.
date <- lubridate::my(cash_rate_table[[1]])
cash_rate <- as.numeric(cash_rate_table[[2]])

# Trim the row headers in column 1.
date <- tail(date, -1)
cash_rate <- tail(cash_rate, -1)
