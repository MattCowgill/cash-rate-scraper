...
rba_meetings <- tibble(
  ...
  event_time = ymd_hm(c(
    ...
    "2026-08-04 14:30", "2026-09-15 14:30", "2026-11-03 14:30",
...
# Keep one scrape per week (latest scrape in each ISO week)
kept_scrapes <- forecast_paths_window %>%
  mutate(scrape_week = floor_date(as.Date(scrape_time), "week")) %>%
